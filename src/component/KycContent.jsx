...
import { webcontentAction, updatekyc_contentAction } from "../Action/action";
...
const KycContent = () => {
  ...
  const updatewebcontent = async (e) => {
    e.preventDefault();
    let res = await updatekyc_contentAction(webcontent);
    console.log("updatewebcontent",res);
    if (res.success) {
      toast.success(res.msg);
      setTimeout(() => {
        window.location.href = `${config.baseUrl}kyccontent`;
      }, 2000);
    } else {
      toast.error(res.msg);
    }
